    ...
    final_thetaonly=[final_thetaonly net_sumrate];
        ...
        for iterates=1:50
            for i=1:k
                ...
                for j=1:k
%                     if(j~=i)
                    hk_=P(j)*abs(hij_array(i,j)'+G_array(:,j)'*diag(theta)*Hri_array(:,i))^2;
                    sum4=sum4+hk_;
%                     end
                end
                betai=(P(i)*sqrt((1+alphas(i))) * (hij_array(i,i)'+ G_array(:,i)'*diag(theta)*Hri_array(:,i)))/sum4;
                betas(i)=betai;
%                 size(theta')
%                 size(diag(Hri_array(:,i)))
%                 size(G_array(:,i))
                hi=hij_array(i,i)+theta'*diag(Hri_array(:,i))*G_array(:,i);
...
%                         sum3=sum3+abs(hij_array(i,j)'+G_array(j)*theta2'*Hri_array(:,j))^2*P(j);
                        sum3=sum3+abs(hij_array(i,j)'+G_array(:,j)'*diag(theta2)*Hri_array(:,i))^2*P(j);
                    end
                end
                
                num1=abs(hij_array(i,i)'+G_array(:,i)'*diag(theta2)*Hri_array(:,i))^2 * P(i);
                gamma=num1/sum3;
...
%             sum5=zeros(1,k);
            ...
            for i=1:k
                ...
                for j=1:k
                    if(j~=i)
                        sum5=sum5+ ((abs(hij_array(i,j)'+G_array(:,j)'*diag(theta2)*Hri_array(:,i))^2)*yi(j)^2)^2;  %no sensor
                        sum3=sum3+abs(hij_array(i,j)'+G_array(:,j)'*diag(theta2)*Hri_array(:,i))^2*P(j);  %no sensor
                    end
                 ...
                 sum7=0;
                 for j=1:k
                    sum6=sum6+(abs(vij_array(i,j)'+G_array(:,i)'*diag(theta2)*Vri_array(:,j))^2)*scaling;
                    sum7=sum7+(abs(vij_array(i,j)'+G_array(:,i)'*diag(theta2)*Vri_array(:,j))^2)*P(i)*scaling;
                 end
%                  sum7
...
%             sum5
            valuetemp=value1;
            value1(i)=(1+gamma)*abs(hij_array(i,i)'+G_array(:,i)'*diag(theta2)*Hri_array(:,i))^2 * abs(yi(i))^2/(sum5);
%             maxpow
%             value1
...
%             value1
            ...
            net_sumrate_thetaonly=sumrate_calc(hij_array,theta,Hri_array,G_array,k,[1 1 1 1]*pow);
...
%             net_sumrate=new_net;
    ...
    if(final(35)~=final(1) && final_poweronly(35)~=final_poweronly(1) && final_thetaonly(35)~=final_thetaonly(1))
        ...
        [x,fval]=ga(@(net_sumrate) sumrate_calc(hij_array,theta2,Hri_array,G_array,k,P),7,optimoptions('ga','MaxGenerations',1000));
        x_=[x_ fval];
        idx=idx+1;
        ...
        for i=1:k
            energy=0;
            for j=1:k
                energy=energy+(abs(vij_array(i,j)'+G_array(:,i)'*diag(theta2)*Vri_array(:,j))^2)*P(i)*scaling;
...
ylabel("Sum Rate")
title('Pmax=',pow)
legend('Theta+Power Optimized','Random Phase','Maximum Power','location','southeast')
