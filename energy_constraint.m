...
%             sum5=zeros(1,k);
            ...
            for i=1:k
                ...
                for j=1:k
                 ...
                 sum7=0;
                 for j=1:k
                    sum6=sum6+(abs(hij_array(i,j)'+G_array(i)*theta2'*Hri_array(:,i))^2);
                    sum7=sum7+(abs(hij_array(i,j)'+G_array(i)*theta2'*Hri_array(:,i))^2)*P(i);
                 end
...
%             value1
            ...
            net_sumrate_thetaonly=sumrate_calc(hij_array,theta2,Hri_array,G_array,k,[1 1 1 1]*pow);
...
%             net_sumrate=new_net;
    ...
    if(final(35)~=final(1) && final_poweronly(35)~=final_poweronly(1) && final_thetaonly(35)~=final_thetaonly(1))
        ...
        for i=1:k
            energy=0;
            for j=1:k
                energy=energy+(abs(hij_array(i,j)'+G_array(i)*theta2'*Hri_array(:,i))^2)*P(i);
